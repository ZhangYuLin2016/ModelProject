...
typedef NS_ENUM(NSInteger,ZYLKeyboardButtonStyle){
    ZYLKeyboardButtonStyleWhite,
    ZYLKeyboardButtonStyleGrey,
    ZYLKeyboardButtonStyleDismiss,
    ZYLKeyboardButtonStyleDone
};

@interface ZYLKeyboardButton : UIButton

+(ZYLKeyboardButton *)keyboardButtonWithStyle:(ZYLKeyboardButtonStyle)style;
...
@property (nonatomic,assign) ZYLKeyboardButtonStyle style;//样式
@end

@implementation ZYLKeyboardButton

+(ZYLKeyboardButton *)keyboardButtonWithStyle:(ZYLKeyboardButtonStyle)style{
    ZYLKeyboardButton *button = [self buttonWithType:UIButtonTypeCustom];
    button.style = style;
    return button;
}
...
- (void)setStyle:(ZYLKeyboardButtonStyle)style{
    if (style != _style) {
        _style = style;
        [self buttonStyleDidChange];
    }
...
- (void)buttonStyleDidChange{
    ...
    if (style == ZYLKeyboardButtonStyleGrey) {
        textColor = [self getColor:@"7D7F82"];
        highlightedTextColor = [UIColor blackColor];
    }else if(style == ZYLKeyboardButtonStyleWhite){
...
+ (id)ZYL_currentFirstResponder
{
    currentFirstResponder = nil;
    [[UIApplication sharedApplication] sendAction:@selector(ZYL_findFirstResponder:) to:nil from:nil forEvent:nil];
    return currentFirstResponder;
}
#pragma clang diagnostic pop

- (void)ZYL_findFirstResponder:(id)sender
{
...
@interface ZYLKeyStyleOne()<UIScrollViewDelegate>
...
- (void)initToolButton{
    ...
    for (int i = 0; i < array.count; i ++) {
        ZYLKeyboardButton *button = [ZYLKeyboardButton buttonWithType:UIButtonTypeCustom];
        ...
        if (i == 0) {
            button.style = ZYLKeyboardButtonStyleGrey;
            ...
            button.style = ZYLKeyboardButtonStyleDone;
            [button addTarget:self action:@selector(dismissKeyboard:) forControlEvents:UIControlEventTouchUpInside];
        }else {
            button.style = ZYLKeyboardButtonStyleGrey;
...
- (id<UIKeyInput>)keyInput
{
    ...
    keyInput = [UIResponder ZYL_currentFirstResponder];
...
- (void)createButtonWithArray:(NSArray *)array columeCount:(NSUInteger)count backspaceButtonIndex:(NSUInteger)index textFont:(UIFont *)font for:(UIView *)superView{
    ...
    float width = kScreenWidth / count;
    float height = HEIGHT / line;
    
    for (int i = 0; i < nameArray.count ; i ++) {
        ZYLKeyboardButton *button = [ZYLKeyboardButton keyboardButtonWithStyle:ZYLKeyboardButtonStyleWhite];
...
- (UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        _scrollView.contentSize = CGSizeMake( 3 * kScreenWidth, HEIGHT);
...
- (UIView *)letterView{
    if (!_letterView) {
        _letterView = [[UIView alloc]initWithFrame:CGRectMake(kScreenWidth, 0, kScreenWidth, HEIGHT)];
...
- (UIView *)numView{
    if (!_numView) {
        _numView = [[UIView alloc]initWithFrame:CGRectMake(2 * kScreenWidth,0 , kScreenWidth, HEIGHT)];
