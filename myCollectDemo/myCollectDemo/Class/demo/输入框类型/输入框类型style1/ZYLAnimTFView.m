...
#pragma mark - ZYLBorderLayer 内部类

/**
 *  ZYLBorderLayer 内部类,提供顶部可变化的边框
 */
@interface ZYLBorderLayer : CAShapeLayer
...
- (instancetype)initWithLayer:(id)layer {
    if ([super initWithLayer:layer]) {
        ZYLBorderLayer *borderLayer = (ZYLBorderLayer *)layer;
 ...
 *  一个输入框,输入文字时又动画效果
 */
@interface ZYLAnimTFView ()<UITextFieldDelegate>
...
@property (nonatomic, strong) ZYLBorderLayer *borderLayer;
    ...
    if ([super initWithFrame:frame]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UITextFieldTextDidBeginEditingNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            if (note.object == self.textField) {
                [self setState:ZYLAnimTextFieldStateEditing];
            }
        }];
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UITextFieldTextDidEndEditingNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            if (note.object == self.textField) {
                [self setState:ZYLAnimTextFieldStateNormal];
            }
        }];
...
- (void)handleTipsTap {
    if (self.inputType == ZYLAnimTextFieldInputTypePassword) {
...
- (void)setState:(ZYLAnimTextFieldState)state {
    if (_state == state && state != ZYLAnimTextFieldStateError) {
        return;
    }
    _state = state;
    switch (state) {
        case ZYLAnimTextFieldStateNormal:
        {
            ...
            break;
        case ZYLAnimTextFieldStateEditing:
        {
            ...
            break;
        case ZYLAnimTextFieldStateError:
        {
            self.placeHolderIV.highlighted = NO;
...
- (void)setInputType:(ZYLAnimTextFieldInputType)inputType {
    _inputType = inputType;
    switch (inputType) {
        case ZYLAnimTextFieldInputTypePassword:
            self.textField.secureTextEntry = YES;
        case ZYLAnimTextFieldInputTypeTips:
...
- (ZYLBorderLayer *)borderLayer {
    if (!_borderLayer) {
        _borderLayer = [ZYLBorderLayer layer];
